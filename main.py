import time

def measure_time(func, *args):
    start_time = time.time()
    result = func(*args)
    end_time = time.time()
    execution_time = end_time - start_time
    return result, execution_time

def addition(poly_1, poly_2):
    poly_1 = poly_1.zfill(233)
    poly_2 = poly_2.zfill(233)
    result = ''
    for i in range(233):
        result += str(int(poly_1[i]) ^ int(poly_2[i]))
    return result.lstrip('0') or '0'

def left_shift(number, shift):
    return (number[shift:] + '0' * shift)[:233]

def mod_polynomial(dividend, divisor):
    dividend = list(map(int, dividend))
    divisor = list(map(int, divisor))
    while len(dividend) >= len(divisor):
        if dividend[0] == 1:
            for i in range(len(divisor)):
                dividend[i] ^= divisor[i]
        dividend.pop(0)
    return ''.join(map(str, dividend)).lstrip('0') or '0'

def mul(a, b, mod):
    a = a.zfill(233)
    b = b.zfill(233)
    result = [0] * (2 * 233)
    for i in range(233):
        if b[232 - i] == '1':
            for j in range(233):
                result[i + j] ^= int(a[232 - j])
    result = ''.join(map(str, result))
    return mod_polynomial(result, mod)

def square(bitstring):
    result = ''
    for bit in bitstring:
        result += bit + '0'
    return result[:233]

def power(poly_1, poly_3, mod):
    result = '1'.zfill(233)
    current_power = poly_1
    for bit in poly_3:
        if bit == '1':
            result = mul(result, current_power, mod)
        current_power = square(current_power)
    return result

def extended_gcd(a, b):
    if b == '0':
        return a, '1', '0'
    else:
        gcd, x1, y1 = extended_gcd(b, mod_polynomial(list(a), list(b)))
        x = y1
        y = addition(x1, mul(y1, mod_polynomial(list(a), list(b)), a))
        return gcd, x, y

def reverse(poly, mod):
    gcd, inv, _ = extended_gcd(mod, poly)
    if gcd != '1':
        raise ValueError("Inverse does not exist")
    return inv.zfill(233)

def trace(poly):
    return str(sum(map(int, poly)) % 2)

poly1_str = "10011111001000100101010100111110100111010110110110111000101001110110000100001100011001010011010110110010111011001110001110000110100111001000000010000100101000111110110001110001111010111000101100111110111111010100100001001101100111000"
poly2_str = "01101011000101101101011101000100100110010000001000000111010110101110101010010001011001000111100000001000101000111110000100100011000110001010000100000000110110001101111110110010001111111101111110101110110110111110101001011101010111101"
poly3_str = '11110011000100111101101011000010011101011010001111110111100111101011000110100000101101110001000010010110101000001000001001000110111001000000000110101001001010111101010100011001000100111011011000111110010010011111110111011011111000011'
mod_str = '11110011000100111101101011000010011101011010001111110111100111101011000110100000101101110001000010010110101000001000001001000110111001000000000110101001001010111101010100011001000100111011011000111110010010011111110111011011111000011'

add_result, add_time = measure_time(addition, poly1_str, poly2_str)
print(f'Addition:  {add_result}')
print(f'Time taken for Addition: {add_time} seconds')

mul_result, mul_time = measure_time(mul, poly1_str, poly2_str, mod_str)
print(f'Multiply:  {mul_result}')
print(f'Time taken for Multiply: {mul_time} seconds')

sq_result, sq_time = measure_time(square, poly1_str)
print(f'Square:  {sq_result}')
print(f'Time taken for Square: {sq_time} seconds')

reverse_result, reverse_time = measure_time(reverse, poly1_str, mod_str)
print(f'Reverse:  {reverse_result}')
print(f'Time taken for Reverse: {reverse_time} seconds')

pow_result, pow_time = measure_time(power, poly1_str, poly3_str, mod_str)
print(f'Power:  {pow_result}')
print(f'Time taken for Power: {pow_time} seconds')
